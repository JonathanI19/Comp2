...
struct student{
    ...
    /// @brief Constructor for struct
    /// @param f (string) first name
    /// @param l (string) last name
    /// @param g (float) gpa
    /// @param i (int) ID
    /// @param m (string) major
    student(string f = "", string l = "", float g = 0.0, int i = 0, string m = "")
    {
        first_name = f;
        last_name = l;
        gpa = g;
...
class hashTable{
    ...
    void collision_handler(student* s, int key){
        int start = key;
        // If current buffer index is nullptr, insert student
    ...
    hashTable(){
        ...
        // Initialize buffer elements to nullptrs
...
void load_data(string filename){
    ...
    while (f_id >> arrival_time){
        ...
        student* temp = new student(first, last, gpa, id, major);
        queue_mutex.lock();

        // Insert student object into queue
        waiting_list.insert(temp);
